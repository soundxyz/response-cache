// Adapted from https://github.com/dotansimha/envelop/blob/main/packages/plugins/response-cache-redis/test/response-redis-cache.spec.ts
...
describe("useResponseCache with Redis cache", () => {
  ...
  test("should indicate if the cache was hit or missed", async () => {
    ...
    const testInstance = createTestkit(
      [useResponseCache({ cache, includeExtensionMetadata: true })],
      schema
    );
  ...
  test("should consider variables when saving response", async () => {
    const spy = vi.fn((_, { limit }: { limit: number }) =>
      [
        {
          id: 1,
          name: "User 1",
  ...
  test("should skip cache of ignored types", async () => {
    ...
    const testInstance = createTestkit(
      [useResponseCache({ cache, ignoredTypes: ["Comment"] })],
      schema
    );
